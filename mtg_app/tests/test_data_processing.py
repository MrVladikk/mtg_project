...
CSV_HEADERS = [
    ...
    'Quantity', 'Scryfall ID', 'Purchase price', 'Condition', 'Language', 'Image URL'
]
...
class ProcessCSVCommandTests(TestCase): # ✅ Inherit from Django's TestCase
    def setUp(self):
        ...
        self.temp_csv_file = tempfile.NamedTemporaryFile(delete=False, mode='w', newline='', encoding='utf-8-sig')
    ...
    def tearDown(self):
        ...
        try:
            if hasattr(self, 'temp_csv_path') and self.temp_csv_path and os.path.exists(self.temp_csv_path): # Проверяем существование пути и файла
            ...
            print(f"Предупреждение: Не удалось удалить временный файл {self.temp_csv_path} в tearDown из-за PermissionError.")
            pass
