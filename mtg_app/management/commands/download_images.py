    ...
    def handle(self, *args, **options):
        ...
        def download_image(scryfall_id, card_name, collector_number):
            ...
            if response.status_code == 200:
                ...
                self.stdout.write(self.style.ERROR(f"Failed to download image for {card_name} (Scryfall ID: {scryfall_id})"))
        ...
        with open(csv_path, newline='', encoding='utf-8') as csvfile:
            ...
            for row in tqdm(rows, total=len(rows), desc="Downloading images"):
                ...
                if not is_image_downloaded(card_name, collector_number):
                    ...
                    self.stdout.write(self.style.WARNING(f"Image for {card_name} (Number: {collector_number}) already exists. Skipping download."))
