    ...
    def handle(self, *args, **options):
        ...
        for card in cards:
            ...
            if os.path.exists(full_path):
                ...
                card.save()
                self.stdout.write(self.style.SUCCESS(f"Обновлено изображение для карты: {card.name} (Number: {card.collector_number}) -> {image_path}"))
            else:
                self.stdout.write(self.style.WARNING(f"Изображение не найдено для карты: {card.name} (Number: {card.collector_number}) (путь: {full_path})"))
