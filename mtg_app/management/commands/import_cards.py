    ...
    def handle(self, *args, **options):
        ...
        for _, row in df.iterrows():
            set_obj, _ = Set.objects.get_or_create(
                code=row["Set code"],
                defaults={"name": row["Set name"]}
            )

            # Преобразуем Purchase price в Decimal
            purchase_price = Decimal(str(row["Purchase price"])) if pd.notnull(row["Purchase price"]) and row["Purchase price"] != "" else Decimal("0.00")

            foil_value = row["Foil"].strip().lower()  # Убираем пробелы и приводим к нижнему регистру
            foil = True if foil_value == "foil" else False  # Преобразуем "foil" в True, а всё остальное в False
